...
const InformeVentas = () => {
  ...
  // --------- RECUPERA DATOS DEL FORM (PERSISTENCIA) ---------
  useEffect(() => {
  ...
  // --------- OBTIENE LAS VENTAS EN EL RANGO ---------
  useEffect(() => {
  ...
  function getResumen(ventas) {
    ...
    let maxCat = 0;
    for (let cat in categoriaCounter) {
      if (categoriaCounter[cat] > maxCat) {
        maxCat = categoriaCounter[cat];
        categoriaMasVendida = cat;
      }
    }

    let productoMasVendido = null;
    let maxProd = 0;
    for (let prod in productoCounter) {
      if (productoCounter[prod] > maxProd) {
        maxProd = productoCounter[prod];
  ...
  // --------- GENERAR PDF (solo tabla y resumen, sin imagen) ---------
  const handleGenerarPDF = () => {
    ...
    doc.setFontSize(12);
    doc.text(`Categoría más vendida: ${resumen.categoriaMasVendida || "N/A"}`, 14, doc.lastAutoTable ? doc.lastAutoTable.finalY + 10 : 55);

    const ventasPorCatText = Object.entries(resumen.ventasPorCategoria || {})
      .map(([cat, cant]) => `${cat}: ${cant} ventas`).join(", ");
    doc.text(`Ventas por categoría: ${ventasPorCatText}`, 14, doc.lastAutoTable ? doc.lastAutoTable.finalY + 18 : 63);
    doc.text(`Producto más vendido: ${resumen.productoMasVendido || "N/A"}`, 14, doc.lastAutoTable ? doc.lastAutoTable.finalY + 26 : 71);

    // Convierte PDF a base64 para backend
  ...
  // --------- ACTUALIZAR PDF ---------
  const handleActualizarPDF = () => {
    setGenerandoPDF(true);
    setTimeout(() => {
      handleGenerarPDF();
      setGenerandoPDF(false);
      setCambiosPendientes(false);
    }, 600); // Puedes ajustar el tiempo según tu gusto
  };

  // --------- UI: FORMULARIO Y VISTA PREVIA ---------
  return (
    <div className="informe-ventas-layout">
      ...
      <div className="informe-ventas-preview">
